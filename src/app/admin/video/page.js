"use client";

import { useState } from "react";

export default function VideoPage() {
    const [videos, setVideos] = useState([
        { id: 1, title: "Intro to Anatomy", type: "MP4", duration: "10 min", createdAt: "Jan 01, 2025" },
        { id: 2, title: "Pharmacology Basics", type: "AVI", duration: "15 min", createdAt: "Jan 05, 2025" },
    ]);

    const handleAdd = () => {
        const newVideo = {
            id: Date.now(),
            title: "New Video",
            type: "MP4",
            duration: "5 min",
            createdAt: new Date().toLocaleDateString("en-US", {
                month: "short",
                day: "2-digit",
                year: "numeric",
            }),
        };
        setVideos([...videos, newVideo]);
    };

    const handlePlay = (title) => {
        alert(`Playing video: ${title}`);
    };

    const handleEdit = (title) => {
        alert(`Editing video: ${title}`);
    };

    const handleDelete = (id) => {
        setVideos(videos.filter((v) => v.id !== id));
    };

    return (
        <div className="font-sans flex flex-col min-h-screen bg-white text-sm text-gray-800 p-6">
            {/* Header */}
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-lg font-semibold text-gray-800">Learning Videos</h1>
                <button
                    onClick={handleAdd}
                    className="px-4 py-2 bg-[#609966] hover:bg-[#4d7c52] text-white rounded-lg text-xs transition"
                >
                    + Add Video
                </button>
            </div>

            {/* Table */}
            <div className="overflow-x-auto bg-white rounded-lg shadow-sm">
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-gray-50 text-gray-700 text-xs">
                            <th className="p-3 text-center">Judul</th>
                            <th className="p-3 text-center">Jenis</th>
                            <th className="p-3 text-center">Durasi</th>
                            <th className="p-3 text-center">Tanggal Upload</th>
                            <th className="p-3 text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        {videos.map((v) => (
                            <tr
                                key={v.id}
                                className="hover:bg-gray-50 transition"
                            >
                                <td className="p-3">{v.title}</td>
                                <td className="p-3 text-[#609966] font-medium">{v.type}</td>
                                <td className="p-3">{v.duration}</td>
                                <td className="p-3">{v.createdAt}</td>
                                <td className="p-3 flex justify-center gap-2 text-xs">
                                    <button
                                        onClick={() => handlePlay(v.title)}
                                        className="px-2 py-1 rounded-md border text-gray-600 hover:bg-gray-100 transition"
                                    >
                                        Play
                                    </button>
                                    <button
                                        onClick={() => handleEdit(v.title)}
                                        className="px-2 py-1 rounded-md border text-[#609966] hover:bg-[#e6f0ea] transition"
                                    >
                                        Edit
                                    </button>
                                    <button
                                        onClick={() => handleDelete(v.id)}
                                        className="px-2 py-1 rounded-md border text-red-600 hover:bg-red-50 transition"
                                    >
                                        Delete
                                    </button>
                                </td>
                            </tr>
                        ))}
                        {videos.length === 0 && (
                            <tr>
                                <td colSpan="5" className="p-4 text-center text-gray-500 text-xs">
                                    No videos uploaded yet.
                                </td>
                            </tr>
                        )}
                    </tbody>
                </table>
                <button
                    onClick={() => (window.location.href = "/admin")}
                    className="fixed bottom-6 right-6 bg-[#609966] text-white px-5 py-3 rounded-full shadow-lg hover:bg-[#4d7c52] flex items-center gap-2 text-sm"
                >
                    Back
                </button>
            </div>
        </div>
    );
}
